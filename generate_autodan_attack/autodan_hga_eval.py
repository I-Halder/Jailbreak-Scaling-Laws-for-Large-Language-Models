...
from utils.opt_utils import get_score_autodan, get_score_autodan_low_memory, autodan_sample_control
    ...
    for i, (g, t) in tqdm(enumerate(dataset), total=len(harmful_data.goal[args.start:])):
        ...
        for j in range(num_steps):
            with torch.no_grad():
                ...
                # updated to address OOM issue for long prompts in harmbench dataset
                try:
                    ...
                    # HarmBench contextual prompts can be much longer; fall back to
                    # per-candidate scoring to avoid large attention tensors.
                    torch.cuda.empty_cache()
                    gc.collect()
                    print("[OOM] Falling back to low-memory scorer for this step.")
