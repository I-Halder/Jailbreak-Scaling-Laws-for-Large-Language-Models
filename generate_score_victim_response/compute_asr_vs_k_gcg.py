...
def main() -> None:
    ...
    victim_model_name = next(
        (part.split("=", 1)[1].split("/")[-1]
         for part in args.model_args.split(",")
         if part.startswith("pretrained=")),
        args.model_args  # fallback: use full string if pretrained= not found
    )
    ...
    # save results to json file at output_path_name
    if args.use_judge_model:
...
if __name__ == "__main__":
    # Usage: python generate_score_victim_response/compute_asr_vs_k_gcg.py --k_min 1 --k_max 6 --k_step 4 --model_args "pretrained=meta-llama/Meta-Llama-3-8B-Instruct,tensor_parallel_size=1,data_parallel_size=1,gpu_memory_utilization=0.9" --output_path ./generation_results/ --tasks advbench --yaml_path "./lm_eval/tasks/advbench/advbench.yaml" --use_judge_model --limit 0.01
    main()