...
def modify_yaml_repeats(yaml_path:str, k:int, victim_model_name:str, judge_model_name:str, attack_type:str='gcg', task_name:str='advbench', run_num:int=0)->None:
    """
...
def run_lm_eval(model_args: str, tasks: str, output_path: str, limit: Optional[float] = None)-> None:
    """
    Runs the lm_eval command with the specified parameters to generate inference time data. Currently suppoting openai-chat-completions and anthropic-chat-completions.
    """
...
def compute_asr_for_k_with_judge(samples_glob: str, judge: JudgeReward, device: torch.device) -> Tuple[float, float, int]:
    """
...
def main() -> None:
    ...
    if not args.skip_lm_eval:
        output_path_name = f"{args.output_path}/task_{args.tasks}_victim_{victim_model_name}_judge_{judge_model_name}_attack_gcg_run_{run_number}"
    else:
    ...
    if not args.skip_lm_eval:
        ...
        for k in k_values:
            ...
            # Modify YAML
            task_name = modify_yaml_repeats(args.yaml_path, k, run_num=run_number, task_name=args.tasks, victim_model_name=victim_model_name, judge_model_name=judge_model_name)
            
            # Run lm_eval
            k_output_path = f"{output_path_name}/k{k}_task_{task_name}"
            run_lm_eval(args.model_args, task_name, k_output_path, args.limit)
    ...
    for k in k_values:
        ...
        if args.use_judge_model:
            asr_mean, asr_std, n_prompts = compute_asr_for_k_with_judge(samples_glob, judge, device)
        else:
    ...
    print(f"\n[Main] Saved plot to {output_path_name}/asr_vs_k_run_{run_number}.png")
    
    # Print summary
    print(f"\n[Main] Summary:")
    print(f"  k values: {k_arr.tolist()}")
    print(f"  asr means: {asr_arr.tolist()}")
    print(f"  Saved: {output_path_name}, k_values.txt, asr_vs_k.txt, asr_vs_k_std.txt")

if __name__ == "__main__":
    # Usage: python generate_score_victim_response/compute_asr_vs_k_gcg_api.py --k_min 1 --k_max 3 --k_step 4 --model_args "model=gpt-4-turbo" --output_path ./generation_results/ --tasks advbench --yaml_path "./lm_eval/tasks/advbench/advbench.yaml" --use_judge_model --limit 0.01
    main()